function setup() {
	...
	critter.lsystem.forEach(function (_lsystem){
		if (_lsystem[0] == "f") { 
			...
			turtle[-1].ctrx = turtle[-1].x + 60 * cos(turtle[-1].th)
			turtle[-1].ctry = turtle[-1].y + 60 * sin(turtle[-1].th)
		}
		else if (_lsystem[0] == "Gc" || _lsystem[0] == "Ge") { 
			for (var i=0; i<1; i+=1/_lsystem[1]){
				var x1 = bezierPoint(turtle[-1].cynx, turtle[-1].ctrx, turtle[-1].x, turtle[-1].x + 60 * cos(turtle[-1].th), i)
				var y1 = bezierPoint(turtle[-1].cyny, turtle[-1].ctry, turtle[-1].y, turtle[-1].y + 60 * sin(turtle[-1].th), i)
				var x2 = bezierPoint(turtle[-1].cynx, turtle[-1].ctrx, turtle[-1].x, turtle[-1].x + 60 * cos(turtle[-1].th), i + 1/_lsystem[1])
				var y2 = bezierPoint(turtle[-1].cyny, turtle[-1].ctry, turtle[-1].y, turtle[-1].y + 60 * sin(turtle[-1].th), i + 1/_lsystem[1])				
